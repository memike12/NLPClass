package usna.assignments;

import usna.util.*;
import usna.langmodel.*;
...
public class LanguageModelTester {
  ...
  static Collection<List<String>> getCorrectSentences(List<JumbleProblem> jumbleProblems) {
	...
	private static void evaluateContext(LanguageModel model, String contextString){
		...
		usna.util.PriorityQueue<String> pq = wordProbabilities.asPriorityQueue();
  ...
  public static void main(String[] args) throws IOException {
    ...
    options.put("-model",     "usna.langmodel.EmpiricalUnigramLanguageModel");
    ...
    if ("true".equals(options.get("-check"))) {
      System.err.println("***********************************\n**** Does the model sum to 1.0 in different contexts?\n***********************************");
    ...
    if ("true".equals(options.get("-generate"))) {
      System.out.println();
      System.out.println("***********************************\n**** Time to generate random sentences! Do these look good?\n***********************************");
      for (int i = 0; i < 10; i++) {
    	  System.out.print("  ");
    	  List<String> tokens = model.generateSentence();
    	  if( tokens.size() > 0 ) {
    	    System.out.print(tokens.get(0).substring(0, 1).toUpperCase() + tokens.get(0).substring(1)); // uppercase first word
